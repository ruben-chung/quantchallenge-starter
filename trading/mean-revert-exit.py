...
Volatility-harvesting version + mean-revert exit (IOC)
"""
...
class Strategy:
    ...
    UPDATED: Volatility-harvesting mode
    ...
    def on_orderbook_update(self, ticker: Ticker, side: Side, quantity: float, price: float) -> None:
        ...
        if self._bid_prices and self._ask_prices:
            ...
            if vwbid > 0 and vwask > 0:
                ...
                # Mean-reversion lean and inventory skew (in ticks → price)
                z = 0.0
                ...
                # Size leaning by displayed imbalance + vol dampen
                tot = bid_vol + ask_vol
                ...
                vol_dampen = 1.0 / (1.0 + self.qty_vol_dampen * max(0.0, sigma_diff) / max(self.tick_size, 1e-6))
                bid_qty = max(self._sanity_qty, bid_qty * vol_dampen)
                ask_qty = max(self._sanity_qty, ask_qty * vol_dampen)
    ...
    def on_order_filled(self, *args, **kwargs) -> None:
        ...
        # Track average entry price for remaining net position
        ...
        # If increasing magnitude in same direction (carry avg); if reducing, keep avg;
        # if crossing through zero, reset avg to current price for the residual.
        if prev_pos == 0.0:
            # opening a new position
            self.pos_avg_price = p
        elif (prev_pos > 0 and new_pos > prev_pos) or (prev_pos < 0 and new_pos < prev_pos):
            # adding to same-direction position → recompute weighted average by absolute size
            ...
            # crossed through zero → new position starts at current price
            self.pos_avg_price = p
        # else: reducing magnitude without crossing zero → keep avg as is
...
#13.8k - very solid, because mid-price i sinheritly very vol.
#13.3k
