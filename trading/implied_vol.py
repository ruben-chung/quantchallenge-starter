...
class Strategy:
    ...
    • Use std(diff(mid)) as a practical intraday IV proxy. Switch between:
      ...
      - MOMO: tighter quoting with slight momentum skew in low IV
    """
    ...
    def reset_state(self) -> None:
        ...
        self.momo_skew: float = 0.20             # small trend-follow lean
        ...
        if self._bid_prices and self._ask_prices:
            ...
            if vwbid > 0 and vwask > 0:
                ...
                if self.regime == "VOL":
                    ...
                    last_diff = 0.0
                    if self._mid_hist and len(self._mid_hist) >= 2:
                        last_diff = self._mid_hist[-1] - self._mid_hist[-2]
                    trend_sign = 1.0 if last_diff > 0 else -1.0 if last_diff < 0 else 0.0

                    center_skew_ticks = (self.momo_skew * trend_sign) + (- self.inv_skew_per_unit * self.position)

                    size_scale = 1.0
                    if abs(self.position) > self.momo_pos_cap:
                        size_scale = 0.5  # bias toward flattening when over cap
