...
Game-theory + session control + mid–PnL adaptation (auto momentum):
- queue-aware quoting
...
- mid–PnL coupling estimator with neutral/momentum modes
- auto switch between neutral and momentum with hysteresis
"""
...
class Strategy:
    """Volume-weighted spread maker, hardened for adversarial bots, with session controls and mid–PnL adaptation."""
    ...
    def reset_state(self) -> None:
        ...
        # Opening probe at start of new game (after cooldown)
        self.open_probe_side: str = "BUY"              # "BUY", "SELL", "NONE"
        self.open_probe_qty: float = 1.0
        self.open_probe_enabled: bool = False
        self._opened_this_game: bool = False

        # ───────── Mid–PnL coupling adaptation ─────────
        # trend_mode: "neutral", "momentum", or "auto" (auto switches using hysteresis)
        self.trend_mode: str = "auto"
        self.k_risk_base: float = 0.02
        self.k_risk_min: float = 0.005
        self.k_risk_max: float = 0.08
        self.pos_target_gain: float = 0.25   # fraction of pos_max as max target when signal=±1
        self.beta_hi: float = 0.6            # strong coupling threshold

        # auto-momentum switch thresholds (normalized momentum score)
        self.auto_mom_on: float = 0.18
        self.auto_mom_off: float = 0.10
        self._last_mode_switch_ts: float = 0.0
        self.auto_min_dwell_sec: float = 5.0

        # online beta/corr estimator (EWMA)
        self.beta_alpha: float = 0.05
        self._mu_mid = 0.0
        self._mu_pnl = 0.0
        self._var_mid = 1e-9
        self._var_pnl = 1e-9
        self._cov_mid_pnl = 0.0
        self.beta_mid_pnl = 0.0
        self.corr_mid_pnl = 0.0
        self._last_mid: Optional[float] = None
        self._last_mtm: Optional[float] = None

        # simple momentum (EWMA of dMid)
        self._ew_dmid = 0.0
        self._mom_alpha = 0.10
    ...
    # ───────── Mid–PnL coupling and momentum helpers ─────────
    def _update_beta_and_signal(self, mid: float) -> None:
        """Update EWMA beta/corr between ΔPnL and Δmid, plus a tiny momentum signal."""
        mtm = self.cash + self.position * mid  # internal mark-to-market
        if self._last_mid is not None and self._last_mtm is not None:
            dmid = mid - self._last_mid
            dpnl = mtm - self._last_mtm
            a = self.beta_alpha

            # EW means
            self._mu_mid = (1 - a) * self._mu_mid + a * dmid
            self._mu_pnl = (1 - a) * self._mu_pnl + a * dpnl

            dm = dmid - self._mu_mid
            dp = dpnl - self._mu_pnl

            # EW cov/vars
            self._cov_mid_pnl = (1 - a) * self._cov_mid_pnl + a * dm * dp
            self._var_mid = (1 - a) * self._var_mid + a * dm * dm
            self._var_pnl = (1 - a) * self._var_pnl + a * dp * dp

            # beta & corr
            self.beta_mid_pnl = self._cov_mid_pnl / max(1e-12, self._var_mid)
            denom = (self._var_mid * self._var_pnl) ** 0.5
            self.corr_mid_pnl = self._cov_mid_pnl / max(1e-12, denom)

            # momentum: EWMA of dmid
            self._ew_dmid = (1 - self._mom_alpha) * self._ew_dmid + self._mom_alpha * dmid

        self._last_mid = mid
        self._last_mtm = mtm

    def _momentum_score(self, micro_skew: float) -> float:
        # combine EWMA(dMid) and microprice skew into a normalized score [-1,1]
        s = self._ew_dmid + 0.5 * micro_skew * self.tick_size
        denom = max(self.tick_size, abs(s))
        score = 0.0 if denom == 0 else self._clamp(s / denom, -1.0, 1.0)
        return score

    def _target_position(self, micro_skew: float) -> float:
        """Momentum target: lean with predicted drift; otherwise 0 (neutral)."""
        if self.trend_mode != "momentum":
            return 0.0
        score = self._momentum_score(micro_skew)
        return self.pos_target_gain * self.pos_max * score

    def _risk_skew(self, micro_skew: float = 0.0) -> float:
        """Use beta-aware k and a target position to steer inventory."""
        pos_tgt = self._target_position(micro_skew)
        if self.trend_mode == "neutral":
            # if ΔPnL tracks Δmid strongly, crank up risk aversion to neutralize
            k = self.k_risk_base * (1.0 + 2.0 * abs(self.beta_mid_pnl))
            k = self._clamp(k, self.k_risk_min, self.k_risk_max)
        else:
            # momentum mode: allow bigger drifts (smaller k)
            k = max(self.k_risk_min, 0.5 * self.k_risk_base)
        return self._clamp(-k * (self.position - pos_tgt), -0.5, 0.5)

    def _maybe_switch_mode(self, micro_skew: float) -> None:
        """Auto flip between neutral and momentum with hysteresis and dwell time."""
        if self.trend_mode not in ("auto", "neutral", "momentum"):
            self.trend_mode = "auto"
        if self.trend_mode != "auto":
            return
        now = time.time()
        if now - self._last_mode_switch_ts < self.auto_min_dwell_sec:
            return
        # don't engage momentum in volatile regimes or when toxicity extreme
        reg = self._regime()
        tox = self._toxicity()
        if reg == "volatile" or abs(tox) > 0.4:
            if self.trend_mode != "neutral":
                self.trend_mode = "neutral"
                self._last_mode_switch_ts = now
            return
        # compute momentum strength
        m = abs(self._momentum_score(micro_skew))
        if isinstance(self.trend_mode, str) and self.trend_mode == "auto":
            # we represent current operative sub-mode using corr_mid_pnl sign;
            # instead: just check thresholds against current internal flag
            pass
        # decide based on hysteresis thresholds
        if getattr(self, "_auto_is_momentum", False):
            if m < self.auto_mom_off:
                self._auto_is_momentum = False
                self.trend_mode = "neutral"
                self._last_mode_switch_ts = now
        else:
            if m > self.auto_mom_on and abs(self.corr_mid_pnl) < 0.95:  # avoid degenerate marking issues
                self._auto_is_momentum = True
                self.trend_mode = "momentum"
                self._last_mode_switch_ts = now
    ...
    def _decide_and_quote(self) -> None:
        ...
        mid = (vwbid + vwask) / 2.0
        # Update beta/corr and momentum signal from the current mid
        self._update_beta_and_signal(mid)
        ...
        # Microprice skew for risk target + maybe switch to momentum
        micro, _mid_na, micro_skew = self._micro_price()
        self._maybe_switch_mode(micro_skew)

        # Risk skew on price and size (beta-aware, optional momentum target)
        r = self._risk_skew(micro_skew)
        ...
        # When actively in momentum mode, bias quoting to accumulate toward target faster
        if self.trend_mode == "momentum":
            score = self._momentum_score(micro_skew)
            accel = 1.0 + 0.3 * abs(score)
            if score > 0:  # up drift → prefer long
                bid_qty = self._clamp(bid_qty * accel, 1.0, self.max_qty)
                ask_qty = max(1.0, ask_qty * (1.0 - 0.2 * abs(score)))
                self.inside_frac = max(0.18, self.inside_frac - 0.02)  # get a bit more aggressive
            elif score < 0:  # down drift → prefer short
                ask_qty = self._clamp(ask_qty * accel, 1.0, self.max_qty)
                bid_qty = max(1.0, bid_qty * (1.0 - 0.2 * abs(score)))
                self.inside_frac = max(0.18, self.inside_frac - 0.02)

        # Auto de-risk when coupling is high in neutral mode
        if self.trend_mode == "neutral" and abs(self.beta_mid_pnl) > self.beta_hi:
            bid_qty *= 0.7
            ask_qty *= 0.7
            self.inside_frac = min(0.5, self.inside_frac + 0.05)
        ...
        if reg == "volatile":
            ...
            # keep whatever momentum adjusted to
            self.inside_frac = max(0.18, min(self.inside_frac, 0.45))
    ...
    def on_game_event_update(self,
        ...
        # During cooldown: keep baselines fresh
